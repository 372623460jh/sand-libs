...
var group = function group(routes) {
  ...
  routes.map(route => {
    var _this$options$locales, _route$meta$group, _route$meta$group2, _route$meta$nav, _route$meta$nav3, _route$meta$group3;
    ...
    let groupTitle = (_route$meta$group2 = route.meta.group) === null || _route$meta$group2 === void 0 ? void 0 : _route$meta$group2.title;
    ...
    if (!groupPath) {
      const parsed = _path().default.parse(route.component); // only process nested route


      if ( // at least 2-level path
      clearPath && clearPath.lastIndexOf('/') !== 0 || // or component filename is the default entry
      parsed && clearPath.length > 1 && new RegExp(`^(index|readme)(\\.(${this.options.locales.map(([name]) => name).join('|')}))?$`, 'i').test(parsed.name)) {
    ...
    if (groupPath) {
      ...
      route.meta.group = route.meta.group || {}; // restore locale prefix or nav path
      ...
      } else if (route.meta.locale && route.meta.locale !== defaultLocale && !groupPath.startsWith(`/${route.meta.locale}`)) {
  ...
  routes.forEach(route => {
    ...
    if (((_route$meta$group4 = route.meta.group) === null || _route$meta$group4 === void 0 ? void 0 : _route$meta$group4.path) && !route.meta.group.title) {
      var _route$meta$nav5;

      route.meta.group.title = // use other same group path title first
      userCustomGroupTitles[route.meta.group.path] || // fallback group title if there only has group path
      route.meta.group.path // discard nav prefix path or locale prefix path
      .replace(((_route$meta$nav5 = route.meta.nav) === null || _route$meta$nav5 === void 0 ? void 0 : _route$meta$nav5.path) || `/${route.meta.locale || ''}`, '') // discard start slash
      .replace(/^\//g, '') // upper case the first english letter
